...
class Command(BaseCommand):
    help = "Seed Security+ questions from CSV"

    def handle(self, *args, **kwargs):
        file_path = os.path.join('exams', 'seed_data', 'security_plus_questions.csv')
        if not os.path.exists(file_path):
            self.stdout.write(self.style.ERROR(f"CSV not found at {file_path}"))
            return

        exam = Exam.objects.get(name__icontains="Security+ Practice Exam")
        ...
        self.stdout.write(self.style.SUCCESS(f"Seeded {count} Security+ questions."))
